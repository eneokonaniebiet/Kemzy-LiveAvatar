...
class SourceRepository(private val context: Context) {
    ...
    fun updateFaceCrop(source: SourceAsset, crop: FaceCrop): SourceAsset =
        source.copy(crop = crop).also(::save)

    fun setFavorite(source: SourceAsset, favorite: Boolean): SourceAsset =
        source.copy(favorite = favorite).also(::save)

    fun recent(): List<SourceAsset> = prefs.getStringSet("recent", emptySet())
        .orEmpty()
        .mapNotNull(::decode)
    ...
    private fun save(asset: SourceAsset) {
        ...
        prefs.edit().putStringSet("recent", values.toList().takeLast(50).toSet()).apply()
    }

    private fun encode(a: SourceAsset) = listOf(
        a.id,
        a.uri.toString(),
        a.kind.name,
        a.crop.left,
    ...
    private fun decode(value: String): SourceAsset? = runCatching {
        ...
        SourceAsset(
            p[0],
            Uri.parse(p[1]),
            SourceKind.valueOf(p[2]),
            FaceCrop(p[3].toFloat(), p[4].toFloat(), p[5].toFloat(), p[6].toFloat()),
            p[7].toBoolean(),
            p[8].toLong()
        )
    }.getOrNull()
