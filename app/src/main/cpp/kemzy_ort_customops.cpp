...
inline float sample(const float* x, int64_t n, int64_t c, int64_t channels, int64_t d, int64_t h, int64_t w,
                    float gx, float gy, float gz, bool alignCorners) {
    const auto coord=[](float g,int64_t size,bool align){return align?((g+1.f)*(size-1))*0.5f:((g+1.f)*size-1.f)*0.5f;};
    ...
    auto at=[&](int64_t zz,int64_t yy,int64_t xx)->float{if(zz<0||zz>=d||yy<0||yy>=h||xx<0||xx>=w)return 0.f;const int64_t idx=(((n*channels+c)*d+zz)*h+yy)*w+xx;return x[idx];};
    const float c000=at(z0,y0,x0),c001=at(z0,y0,x1),c010=at(z0,y1,x0),c011=at(z0,y1,x1),c100=at(z1,y0,x0),c101=at(z1,y0,x1),c110=at(z1,y1,x0),c111=at(z1,y1,x1);
    const float c00=c000+(c001-c000)*wx,c01=c010+(c011-c010)*wx,c10=c100+(c101-c100)*wx,c11=c110+(c111-c110)*wx,a=c00+(c01-c00)*wy,b=c10+(c11-c10)*wy;return a+(b-a)*wz;
}

OrtStatusPtr ORT_API_CALL compute(void* kernelPtr, OrtKernelContext* ctx){
    const OrtApi* api=static_cast<Kernel*>(kernelPtr)->api;const OrtValue*xv=nullptr,*gv=nullptr;if(auto s=api->KernelContext_GetInput(ctx,0,&xv))return s;if(auto s=api->KernelContext_GetInput(ctx,1,&gv))return s;
    OrtTensorTypeAndShapeInfo*xs=nullptr,*gs=nullptr;if(auto s=api->GetTensorTypeAndShape(xv,&xs))return s;if(auto s=api->GetTensorTypeAndShape(gv,&gs)){api->ReleaseTensorTypeAndShapeInfo(xs);return s;}
    size_t xr=0,gr=0;api->GetDimensionsCount(xs,&xr);api->GetDimensionsCount(gs,&gr);if(xr!=5||gr!=5){api->ReleaseTensorTypeAndShapeInfo(xs);api->ReleaseTensorTypeAndShapeInfo(gs);return api->CreateStatus(ORT_INVALID_ARGUMENT,"GridSample3D requires 5-D input and grid");}
    int64_t xd[5],gd[5];api->GetDimensions(xs,xd,5);api->GetDimensions(gs,gd,5);if(gd[4]!=3||xd[0]!=gd[0]){api->ReleaseTensorTypeAndShapeInfo(xs);api->ReleaseTensorTypeAndShapeInfo(gs);return api->CreateStatus(ORT_INVALID_ARGUMENT,"GridSample3D expects grid shape [N,D,H,W,3]");}
    const int64_t outShape[5]={xd[0],xd[1],gd[1],gd[2],gd[3]};OrtValue*out=nullptr;if(auto s=api->KernelContext_GetOutput(ctx,0,outShape,5,&out)){api->ReleaseTensorTypeAndShapeInfo(xs);api->ReleaseTensorTypeAndShapeInfo(gs);return s;}
    void*xp=nullptr,*gp=nullptr,*op=nullptr;api->GetTensorMutableData(const_cast<OrtValue*>(xv),&xp);api->GetTensorMutableData(const_cast<OrtValue*>(gv),&gp);api->GetTensorMutableData(out,&op);const float*x=static_cast<const float*>(xp),*grid=static_cast<const float*>(gp);float*y=static_cast<float*>(op);
    const int64_t N=xd[0],C=xd[1],D=xd[2],H=xd[3],W=xd[4],OD=gd[1],OH=gd[2],OW=gd[3];
    for(int64_t n=0;n<N;++n)for(int64_t c=0;c<C;++c)for(int64_t z=0;z<OD;++z)for(int64_t yy=0;yy<OH;++yy)for(int64_t xx=0;xx<OW;++xx){const int64_t gi=(((n*OD+z)*OH+yy)*OW+xx)*3;const float v=sample(x,n,c,C,D,H,W,grid[gi],grid[gi+1],grid[gi+2],false);const int64_t oi=((((n*C+c)*OD+z)*OH+yy)*OW+xx);y[oi]=v;}
